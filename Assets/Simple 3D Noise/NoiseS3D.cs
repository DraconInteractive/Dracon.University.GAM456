...
public static class NoiseS3D {
    ...
    private static void SetupNoise() {
        p = new int[256];
        for(int i = 0; i < 256; i++) p[i] = UnityEngine.Mathf.FloorToInt(UnityEngine.Random.value * 256);
 ...
 new int[]{2,1,0,3}, new int[]{0,0,0,0}, new int[]{0,0,0,0}, new int[]{0,0,0,0}, new int[]{3,1,0,2}, new int[]{0,0,0,0}, new int[]{3,2,0,1}, new int[]{3,2,1,0}};
    ...
    public static int octaves {
        ...
        set {
            octaves_ = UnityEngine.Mathf.Clamp(value, 1, 10);
        }
    }
    ...
    /// <param name="noiseScale"> Value to scale the noise coordinates by. </param>
    /// <param name="normalize"> Whether or not to remap the noise from (-1, 1) to (0, 1). </param>
    public static float[] NoiseArrayGPU(UnityEngine.Vector2[] positions, float noiseScale = 0.01f, bool normalize = true) {
        UnityEngine.ComputeShader shader = UnityEngine.Resources.Load(shaderPath) as UnityEngine.ComputeShader;
        if(shader == null) {
            UnityEngine.Debug.LogError(noShaderMsg);
            return null;
        ...
        UnityEngine.ComputeBuffer posBuffer = new UnityEngine.ComputeBuffer(positions.Length, 8);
        posBuffer.SetData(positions);
    ...
    /// <param name="noiseScale"> Value to scale the noise coordinates by. </param>
    /// <param name="normalize"> Whether or not to remap the noise from (-1, 1) to (0, 1). </param>
    public static float[] NoiseArrayGPU(UnityEngine.Vector3[] positions, float noiseScale = 0.01f, bool normalize = true) {
        UnityEngine.ComputeShader shader = UnityEngine.Resources.Load(shaderPath) as UnityEngine.ComputeShader;
        if(shader == null) {
            UnityEngine.Debug.LogError(noShaderMsg);
            return null;
        ...
        UnityEngine.ComputeBuffer posBuffer = new UnityEngine.ComputeBuffer(positions.Length, 12);
        posBuffer.SetData(positions);
